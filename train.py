...
def cnn(x, act = tf.nn.selu, reuse = False, name = '3DCNN', is_train = True):
    with tf.variable_scope(name, reuse=reuse):
        ...
        network = tl.layers.Conv2dLayer(prev_layer=network, act = tf.identity, shape=[filter_shape, filter_shape, feature, feature], strides=[1, 1, 1, 1],
...
nf.customfit(sess = sess, network = network, cost = cost, train_op = train_op, tra_provider = tra_provider, x = x, y_ = y_, acc=acc, n_epoch=EPOCH,
              ...
              save_path='/oldhome/s1500945/etri/model/cnn_10', epoch_identifier=None, baseline = 100000)
# sess.close()
