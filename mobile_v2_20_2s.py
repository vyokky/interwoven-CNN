...
def mobilenet_v2(layer, n_filter, stride=1, filtersize = (filter_shape, filter_shape), name = 'mobile_2', is_train = True, reuse=False, act=tf.nn.relu6, expand_ratio = 6, shortcut = True):
    with tf.variable_scope(name, reuse=reuse):
        network = tl.layers.Conv2d(layer, n_filter = n_filter*expand_ratio, filter_size = (1, 1), name=(name +'cnn1'), act=tf.identity)
        network = tl.layers.BatchNormLayer(network, act=act, is_train=is_train, name=(name +'_bn1'))
        network = tl.layers.DepthwiseConv2d(network, shape = filtersize, strides = (1, stride, stride, 1), name=(name + '_dwise1'), act=tf.identity, padding='SAME', depth_multiplier=1)
        network = tl.layers.BatchNormLayer(network, act=act, is_train=is_train, name=(name +'_bn2'))
        network = tl.layers.Conv2d(network, n_filter = n_filter, filter_size = (1, 1), name=(name +'cnn2'), act=tf.identity)
        network = tl.layers.BatchNormLayer(network, act=tf.identity, is_train=is_train, name=(name +'_bn3'))
        if stride == 1 and shortcut:
            network = tl.layers.ElementwiseLayer([layer, network], combine_fn=tf.add, name=name + 'add')
        return network
...
def combine(l1, l2, reuse=False, name='combine', is_train=True):
    with tf.variable_scope(name, reuse=reuse):
        ...
        network =  mobilenet_v2(network, feature, name='mobile8')
        network =  mobilenet_v2(network, feature, name='mobile9')
        network =  mobilenet_v2(network, feature, name='mobile10')
        network =  mobilenet_v2(network, feature, name='mobile11')
        network =  mobilenet_v2(network, feature, name='mobile12')
        network =  mobilenet_v2(network, feature, name='mobile13')
        network =  mobilenet_v2(network, feature, name='mobile14')
        network =  mobilenet_v2(network, feature, name='mobile15')
        network =  mobilenet_v2(network, feature, name='mobile16')
        network =  mobilenet_v2(network, feature, name='mobile17')
        network =  mobilenet_v2(network, feature, name='mobile18')
        network =  mobilenet_v2(network, feature, name='mobile19')
        network =  mobilenet_v2(network, feature, stride=2, name='mobile20')
...
nf.customfit_2(sess = sess, network = network, cost = cost, train_op = train_op, tra_provider = tra_provider, x = x, x2 = x2, y_ = y_, acc=acc, n_epoch=EPOCH,
              ...
              save_path='/oldhome/s1500945/etri/model/mobilenet_v2_2s', epoch_identifier=None, baseline = 100000)
# sess.close()
